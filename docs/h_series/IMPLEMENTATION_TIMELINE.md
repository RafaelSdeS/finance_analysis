...
| # | Stage | What | How | Why |
|---|-------|------|-----|-----|
| 0 | Data question | Answered, not open — `features.py::_load_daily_prices()` already provides it | Reused verbatim, no new source needed; only remaining check is that the eligible-ticker universe aligns exactly between that daily frame and the monthly decision panel | Blocks H3 stage 3 until the one remaining universe-alignment check is verified |
