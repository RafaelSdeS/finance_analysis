...
- [x] `src/portfolio/pipeline.py::make_full_weights_fn` wires α (2.6) → Σ (2.5) → optimizer (2.4) into one `weights_fn`; `run_full_backtest.py` runs it purged/embargoed, end to end, through the harness (2.3), on the real dataset.
- [x] Full §8 panel reported: deflated Sharpe, turnover/holding-period, regime slices, cost-sensitivity curve (one-way 0.03%→0.15%→0.3%, round-trip shown alongside), vs all three baselines.
- [x] **Two real bugs found and fixed while producing the real-data result** (both concretely diagnosed, not guessed at):
  1. **A single market-wide data gap (2026-05-22) was disqualifying ~49/50 tickers from the risk model every quarter whose trailing window touched it**, because `shrinkage_cov`'s "drop a column on ANY NaN" policy is unforgiving of even one missing day. Confirmed by direct inspection (49/50 tickers NaN on that exact date, despite it being a real trading day for others) — fixed with a bounded (5-day) forward-fill in `pipeline.py` before computing returns, tight enough to still exclude a genuinely-delisted name.
  2. **A real units mismatch**: `shrinkage_cov` estimates a *daily* covariance, but α is a *252-day* cumulative excess return — feeding the optimizer a daily Σ against an annual α made the risk-aversion term `(λ/2)wᵀΣw` negligible (daily variance ~0.0003–0.0008 vs. α on the order of several % to tens of %, confirmed ~100–300x apart by direct measurement), so the optimizer effectively ignored risk and chased whichever ticker had the highest apparent (noisy) α each quarter — the *opposite* of the proposal's own "L1 creates a no-trade band" thesis, which §2.4 had already verified holds correctly in an isolated, properly-scaled toy case. Fixed by scaling Σ by `horizon_td` (`scaled_sigma()`, new function, directly tested: exactly 252× the raw daily covariance, not just an indirect behavioral check — an initial indirect test was itself unreliable, see the test file). **This is the kind of cross-module integration bug that only surfaces when the pieces are actually wired together** — each of Σ (2.5) and the optimizer (2.4) tested correct in isolation.
- [x] **Honest final result (2026-07-24, after both fixes) — the panel prints, and the strategy does NOT beat the equal-weight floor:**
  | | Ann. return | Sharpe | Annual turnover | Max DD |
  |---|---|---|---|---|
  | Equal-weight (2.3 floor) | 14.0% | 0.65 | 0.89 | −62.5% |
  | BOVA11 buy-and-hold | 9.5% | 0.49 | — | −49.7% |
  | 100% CDI | 12.0% | 41.7 (riskless) | — | 0.0% |
  | **Full pipeline** (c₁=0.03%, w_max=0.1, λ=1) | 9.9% | 0.48 | **4.16** | −67.9% |

  Annual turnover of ~4.16 (vs. the floor's 0.89) persisted **even after both fixes** — traced to the α signal itself being weak (out-of-sample rank-IC ≈ 0.056, §2.6) combined with a tight 10%-cap forcing the optimizer to hold ~10 names, whose *marginal* (9th/10th-ranked) members shift under a noisy signal even with a correctly no-trade-banded optimizer. **Not tuned further to look better** — sweeping λ/`w_max`/c₁ post-hoc until the number improves would be exactly the single-history overfitting §9.1 warns against. This is reported as the honest, as-configured result of a correctly-wired pipeline, not a bug to paper over.
- **Done — 2026-07-24.** Per the Done-when criterion's own wording: the panel prints out-of-sample, and the strategy's relationship to the CDI floor is stated plainly — it does not beat equal-weight, BOVA11, or (net of realistic cost) CDI at these settings. That is a valid, honest V1 answer, not a failure of the exercise: the point of V1 was to prove the machinery end to end (α → Σ → optimizer → harness, no lookahead, feasible weights, a verified no-trade band) before spending effort on hyperparameter search or a richer α model — both explicitly deferred (§10, Phase 7+).
