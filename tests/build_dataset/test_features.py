...
def test_adj_close_precision_degraded_flag() -> None:
    ...
    silently rounded away into a spurious flat log_return. Also flags NaN
    and exact-zero adj_close (§2a, DATA_LAYER_CORRECTNESS_PLAN.md): these
    used to slip through unflagged (NaN>0 and 0.0>0 are both False, so the
    old near_floor-only definition could never fire on either), which let
    them reach ml_dataset.parquet looking untouched even though
    validate_prices explicitly excludes adj_* from its own NaN check on the
    assumption "already flagged downstream via this column". Must NOT flag
    a genuinely tiny but full-precision value (e.g. TIMS3-style 0.000568 --
    a real number, not a rounding artifact, confirmed 2026-07-14 by checking
    it's never exactly equal to its own 2-decimal rounding)."""
    df = pd.DataFrame({
        "ticker": ["A"] * 7,
        "trade_date": pd.date_range("2026-01-01", periods=7),
        "adj_close": [0.0, 0.03, 0.04, 0.000568, 6.20, 100.0, np.nan],
        "adj_high": [0.0, 0.03, 0.04, 0.000568, 6.20, 100.0, np.nan],
        "adj_low": [0.0, 0.03, 0.04, 0.000568, 6.20, 100.0, np.nan],
        "adj_open": [0.0, 0.03, 0.04, 0.000568, 6.20, 100.0, np.nan],
        "volume": 1_000_000.0,
        "traded_amount": 100_000_000.0,
    })
    result = compute_price_features(df)

    assert list(result["adj_close_precision_degraded"]) == [1, 1, 1, 0, 0, 0, 1]
