...
- **Reward, cash-aware benchmark (updated 2026-07-09, roadmap M3.1):** excess-return signal = (portfolio log return) − (cash-aware benchmark log return) − transaction_cost, where the benchmark = `w_cash · SELIC + (1 − w_cash) · equal-weight-equity`, using the AGENT'S OWN current cash weight `w_cash` (drifted, day-by-day). This replaces the earlier pure-equity-EW benchmark, under which going 100% CASH during any market downturn earned large positive reward regardless of stock-picking skill (verified: a 100%-CASH action now yields reward ≈ 0, not a large positive number — see `tests/agent/test_env_basic.py` test 7b). Because the benchmark always holds the same cash fraction as the agent, cash timing is reward-neutral by construction; reward now only comes from deviating the EQUITY SLEEVE away from equal-weight (genuine cross-sectional stock selection). Cost = `transaction_cost_bps / 10000 × one-way-turnover` (excluding CASH leg which trades free). Shaping (July 9, 2026): daily reward = excess − `risk_aversion`·excess² (mean-variance penalty, λ=5, prefers smooth alpha) and the per-step sum is multiplied by `reward_scale` (100, conditions PPO's value loss; raw reward was ~±0.02/step). Episodes start/reset at 100% CASH (new investor), so the first allocation pays full deployment cost. Backtest/eval metrics (`metrics.py`) are computed from `info`'s absolute daily returns, independent of this internal reward shaping.
